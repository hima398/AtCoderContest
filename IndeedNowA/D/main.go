...
func Solve(h, w int, c [][]int) int {
	type Cell struct {
		i, j int
	}
	ComputeManhattanDist := func(i1, j1, i2, j2 int) int {
		return Abs(i2-i1) + Abs(j2-j1)
	}
	m := make([][2]int, h*w)
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			m[w*i+j][0], m[w*i+j][1] = i, j
		}
	}
	//fmt.Println(m)
	var ans int
	for i := 0; i < h; i++ {
		for j := 0; j < w; j++ {
			if c[i][j] > 0 {
				ans += ComputeManhattanDist(i, j, m[c[i][j]-1][0], m[c[i][j]-1][1])
				//fmt.Println(ans, c[i][j], i, j, m[c[i][j]-1][0], m[c[i][j]-1][1])
			}
		}
	}
	return ans
