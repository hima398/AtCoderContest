...
func SolveHonestly(n int, a []int) (anses [][]int) {
	m := (1 << n) - 1
	g, s := 1.0, 0.0
	gMax := 0.0
	var ansPat []int
	for pat := 0; pat <= m; pat++ {
		for i := 0; i < n; i++ {
			if (pat>>i)&1 == 1 {
				g, s = s/float64(a[i]), g*float64(a[i])
			}
		}
		if g >= gMax {
			gMax = g
			//ansPat = append(ansPat, pat)
		}
	}
	g, s = 1.0, 0.0
	for pat := 0; pat <= m; pat++ {
...
func FirstSolve(n int, a []int) (ans []int) {
	...
	for i := 1; i <= n; i++ {
		// 前日の金を持ってくるか、前日の銀を交換
		dp[i][0] = math.Max(dp[i-1][0], dp[i-1][1]/float64(a[i-1]))

		// 前日の金を交換するか、前日の銀を持ってくる
		dp[i][1] = math.Max(dp[i-1][1], dp[i-1][0]*float64(a[i-1]))
	...
	for i := n; i >= 1; i-- {
		...
		if dp[i][0] == dp[i-1][0] && dp[i][1] == dp[i-1][1] {
			...
			// dp[i][0]は dp[i-1][0]を引き継ぐか、dp[i-1][1]を換金するか
			if dp[i-1][0] > dp[i-1][1]/float64(a[i-1]) {
				ans[i-1] = 0
				OddEven ^= 1 //交換しない場合、金銀の気にするポイントが変わるのでこれを挟んでおく
			} else {
				ans[i-1] = 1
			}
		} else {
			//銀を増やせるなら交換
			if dp[i-1][0]*float64(a[i-1]) > dp[i-1][1] {
				ans[i-1] = 1
			} else {
				ans[i-1] = 0
				OddEven ^= 1 //交換しない場合、金銀の気にするポイントが変わるのでこれを挟んでおく
