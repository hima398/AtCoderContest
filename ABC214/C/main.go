...
type PriorityQueue []Sunuke

func (pq PriorityQueue) Len() int {
	return len(pq)
}
func (pq PriorityQueue) Less(i, j int) bool {
	return pq[i].ans < pq[j].ans
}
...
func FirstSolve() {
	...
	snk := make([]Sunuke, n)
	for i := 0; i < n; i++ {
		snk[i] = Sunuke{i, s[i], t[i], INF}
	}
	sort.Slice(snk, func(i, j int) bool {
		return snk[i].ans < snk[j].ans
	})
	ans := make([]int, n)
	for i := 0; i < n; i++ {
		ans[i] = INF
	...
	for i := 0; i < n; i++ {
		//もらったのが早いすぬけくん
		idx := snk[i].i
		ans[idx] = Min(ans[idx], snk[i].t) // 宝石をもらう時間
		//隣のすぬけ君に渡す
		ni := (snk[i].i + 1) % n
		ans[ni] = Min(ans[ni], ans[idx]+snk[i].s)
	}
...
func main() {
	...
	q := &PriorityQueue{}
	for i := 0; i < n; i++ {
		snk[i] = Sunuke{i, s[i], t[i], t[i]}
		q.Push(snk[i])
	}
	sort.Slice(snk, func(i, j int) bool {
		return snk[i].t < snk[j].t
	})
	ans := make([]int, n)
	for i := 0; i < n; i++ {
		ans[i] = INF
	...
	for q.Len() > 0 {
		...
		if ans[idx] > su.ans {
			ans[idx] = su.ans
			q.Push(Sunuke{su.i, su.s, su.t, ans[idx]})
		}
		ni := (idx + 1) % n
		if ans[ni] > ans[idx]+s[idx] {
			ans[ni] = ans[idx] + s[idx]
			q.Push(Sunuke{ni, s[ni], t[ni], ans[ni]})
		}
	}
	out := bufio.NewWriter(os.Stdout)
	defer out.Flush()
