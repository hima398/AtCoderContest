...
func FirstSolve(n int, a, b []int) float64 {
	s := make([]float64, n+2)
	ss := make([]int, n+2)
	//sa, sb := make([]float64, n+2), make([]float64, n+2)
	//sa[0], sb[0] = float64(a[0]), float64(b[0])
	for i := 1; i <= n; i++ {
		s[i] += s[i-1] + float64(a[i])/float64(b[i])
		ss[i] += ss[i-1] + a[i]
	}
	s[n+1] = s[n]
	idx := 1
	...
	fmt.Println(idx, s[idx-1], s[idx+1]-s[idx])
	//d := (s[idx-1]*float64(b[idx]) + float64(a[idx]) + (s[idx+1]-s[idx])*float64(b[idx])) / 2
	d := s[idx+1] - s[idx] - s[idx-1]
	fmt.Println(d)
	dd := (float64(a[idx]) + d*float64(b[idx])) / 2.0
	//if d > 0 {
	//	dd = d * float64(b[idx])
	//} else {
	//	dd = (float64(a[idx]) + d*float64(b[idx])) / 2
	//}
	return float64(ss[idx-1]) + dd
}
func Solve(n int, a, b []int) float64 {
	t := 0.0
	st := make([]float64, n)
	for i:=0; i<n; i++ {
		st[i] += float64(a[i])/float64(b[i])
		t+=st[i]
	}
	t /= 2.0
	ans := 0.0
	for i:=0; i<n; i++ {
		ans += math.Min(float64(a[i]), t*float64(b[i]))
		t -= math.Min(st[i], t)
	}
	return ans
}
