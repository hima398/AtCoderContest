...
func Solve(x, y, r float64) int {
	a := math.Abs(x+r-(x-r)) * math.Abs(y+r-(y-r))
}
...
func main() {
	...
	x, y, r := nextFloat64(), nextFloat64(), nextFloat64()
	Solve(x, y, r)
	/*
		fmt.Println(x-r, x+r, y-r, y+r)
		points := 0
		yy := int(r)
		for xx := 0; xx <= int(r); xx++ {
			for xx*xx+yy*yy > int(r)*int(r) {
				yy--
			}
			points += yy
		}
