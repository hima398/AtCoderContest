...
func Solve(m int, u, v, p []int) int {
	e := make([][]int, 10)
	for i := 0; i < m; i++ {
	...
	sp := strings.Split(strings.Repeat("9", 9), "")
	for i, v := range p {
		sp[v-1] = string(i + 1 + '0')
	}
	//fmt.Println(p, sp)
	pattern := make(map[string]int)
	pattern[strings.Join(sp, "")] = 0
	var q []string
	q = append(q, strings.Join(sp, ""))
	for len(q) > 0 {
		s := q[0]
		//fmt.Println(s)
		ss := strings.Split(s, "")
		...
		for _, to := range e[space] {
			ts := make([]string, 10)
			copy(ts, ss)
			ts[space-1], ts[to-1] = ts[to-1], ts[space-1]
			t := strings.Join(ts, "")
			// すでに作成済みのパターン
			if _, visited := pattern[t]; visited {
				continue
			}
			pattern[t] = pattern[s] + 1
			q = append(q, t)
		}
	}

	ans, visited := pattern["123456789"]
	if !visited {
		return -1
	} else {
		return ans
