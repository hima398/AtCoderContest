	...
	for i := 0; i < n-2; i++ {
		for j := i + 1; j < n-1; j++ {
			for k := j + 1; k < n; k++ {
				...
				//var s []int
				//s = append(s, ComputeDist2(x[i], y[i], x[j], y[j]))
				//s = append(s, ComputeDist2(x[i], y[i], x[k], y[k]))
				//s = append(s, ComputeDist2(x[j], y[j], x[k], y[k]))
				//sort.Ints(s)
				//d1 := ComputeDist(x[i], y[i], x[j], y[j])
				//d2 := ComputeDist(x[i], y[i], x[k], y[k])
				//d3 := ComputeDist(x[j], y[j], x[k], y[k])

				//if math.Abs(d1-d2) < d3 && d3 < d1+d2 {
				//	ans++
				//}
				...
				//}
				//fmt.Println(s)
				//if math.Sqrt(float64(s[2])) < math.Sqrt(float64(s[0]))+math.Sqrt(float64(s[1])) {
				//	ans++
				//}
			}
		}
