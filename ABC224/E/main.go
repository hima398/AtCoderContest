...
func Solve(h, w, n int, r, c, a []int) []int {
	...
	for _, ai := range as {
		for _, cell := range cs[ai] {
			dp[cell.i] = Max(dph[cell.r], dpw[cell.c])
		}
		for _, cell := range cs[ai] {
			dph[cell.r] = Max(dph[cell.r], dp[cell.i]+1)
			dpw[cell.c] = Max(dpw[cell.c], dp[cell.i]+1)
		}
	}
	return dp
}
...
func main() {
	...
	r, c, a := make([]int, n), make([]int, n), make([]int, n)
	for i := 0; i < n; i++ {
		r[i], c[i], a[i] = nextInt(), nextInt(), nextInt()
	}
