...
func main() {
	...
	ans := make([]int, 3*n)
	//グループの番号
	mm := map[string]int{"ABC": 1, "ACB": 2, "BAC": 3, "BCA": 4, "CAB": 5, "CBA": 6}
	...
	Check := func(k string, n int) bool {
		ok1, ok2 := false, false
		fi, si, ti := k[0]-'A', k[1]-'A', k[2]-'A'
		for i := 0; i < 3*n; i++ {
			ok1 = ok1 || ((e[si][i] > 0 && Max((e[fi][i]-n), 0) <= e[si][i]) && (e[ti][i] > 0 && Max(e[fi][i]-n, 0) <= e[ti][i]))
			ok2 = ok2 || (e[ti][i] > 0 && Max((e[si][i]-n), 0) <= e[ti][i] && e[fi][i]-n > 0)
		}
		return ok1 && ok2
	}
	...
	for len(mm) > 0 && nn > 0 {
		...
		for i < 3 {
			gn := 0
			for j := 0; j < 3*n; j++ {
				//fmt.Println(kmax, nmax, i, j)
				if s[j] == kmax[i] && ans[j] == 0 {
					ans[j] = gid
					gn++
				}
				if gn >= nmax {
		...
		for i := 0; i < 3; i++ {
			for j := 0; j < 3*n; j++ {
				e[i][j] = Max(e[i][j]-nmax, 0)
			}
		}

		nn -= nmax
		delete(mm, kmax)
		fmt.Println(nmax, kmax, nn, ans)
	}
	// 出力
