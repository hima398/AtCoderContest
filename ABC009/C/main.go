...
func SecondSolve(n, k int, s string) string {

	ret := strings.Split(s, "")
	CanSwap := func(dst string) bool {
		diff := 0
		for i := 0; i < n; i++ {
			if dst[i] != s[i] {
				diff++
...
func FirstSolve(n, k int, s string) string {
	m := make(map[string]int)
	for _, c := range s {
		m[string(c)]++
	}
	ans := ""
	Enable := func(c string) bool {
		...
		//for i := 0; i < 26; i++ {
		//	key := string(i + 'a')
		//	kk2 += Min(m1[key], m2[key])
		//}
		for key := range m1 {
			kk2 += Abs(m1[key] - m2[key])
		}
		fmt.Println(kk, n, len(ans), kk2, k)
...
func main() {
	...
	n, k := nextInt(), nextInt()
	s := nextString()
	//ans := FirstSolve(n, k, s)
	ans := SecondSolve(n, k, s)
	fmt.Println(ans)
}
