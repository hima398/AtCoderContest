...
func SolveHonestly(k, m int, a, c []int) int {
	for i := k; i < m; i++ {
		v := 0
		for j := 0; j < k; j++ {
			v ^= c[j] & a[i-j-1]
...
func MulMatrix(n int, a, b [][]int) [][]int {
	ret := make([][]int, n)
	for i := 0; i < n; i++ {
		ret[i] = make([]int, n)
	}
...
func PowMatrix(n int, a [][]int, x int) [][]int {
	ret := make([][]int, n)
	for i := 0; i < n; i++ {
		ret[i] = make([]int, n)
	}
...
func Solve(k, m int, a, c []int) int {
	cm := make([][]int, k)
	for i := 0; i < k; i++ {
		cm[i] = make([]int, k)
	}
