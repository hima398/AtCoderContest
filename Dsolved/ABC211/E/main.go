...
func main() {
	...
	m := make(map[int]bool)
	ans := 0
	var Dfs func(i, j, used int, visited [][]bool)
	Dfs = func(i, j, used int, visited [][]bool) {
		used |= ConvertBits(i, j)
		//kマスの連結成分まで探索できたので答えをインクリメントする
		if bits.OnesCount(uint(used)) == k {
			if !m[used] {
				ans++
				m[used] = true
			}
		...
		for k := 0; k < 4; k++ {
			...
			if ni >= 0 && ni < n && nj >= 0 && nj < n {
				if s[ni][nj] != '#' && !visited[ni][nj] {
					nvisited := make([][]bool, n)
					for ii := 0; ii < n; ii++ {
						nvisited[ii] = make([]bool, n)
						copy(nvisited[ii], visited[ii])
					}
					nvisited[ni][nj] = true
					Dfs(ni, nj, used, nvisited)
				}
			}
		}
	}
