...
func SolveHonestly(n, x, y int, a, b []int) int {

	const INF = 1 << 60
	const MAX = 300
	var dp [MAX + 1][2*MAX + 1][2*MAX + 1]int
	for i := 0; i <= MAX; i++ {
		for j := 0; j <= 2*MAX; j++ {
			for k := 0; k <= 2*MAX; k++ {
				dp[i][j][k] = INF
			...
			for k := 0; k <= 2*MAX; k++ {
				...
				if nj <= 2*MAX {
					if nk <= 2*MAX {
						...
						dp[i][nj][k] = Min(dp[i][nj][k], dp[i-1][j][k]+1)
					}
				}
				if nk <= 2*MAX {
					// nj<=MAXのケースは上で評価済み
					dp[i][j][nk] = Min(dp[i][j][nk], dp[i-1][j][k]+1)
				}
			}
		}
	}
	//fmt.Println(dp[1][2][1])
	ans := INF
	for j := x; j <= 2*MAX; j++ {
		for k := y; k <= 2*MAX; k++ {
