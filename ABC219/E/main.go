...
func Check1(x int) bool {
	...
	p2 := (((1<<8 - 1) ^ (1<<4 - 1)) & x) >> 4
	p3 := (((1<<12 - 1) ^ (1<<8 - 1)) & x) >> 8
	p4 := (((1<<16 - 1) ^ (1<<12 - 1)) & x) >> 12
	//fmt.Printf("%04b\n", p1)
	//fmt.Printf("%04b\n", p2)
	//fmt.Printf("%04b\n", p3)
	//fmt.Printf("%04b\n", p4)

	return p1&p2 > 0 && p2&p3 > 0 && p3&p4 > 0
}
...
search_start:
	...
	for len(q1) > 0 {
		...
		for k := 0; k < 4; k++ {
			...
			if ni >= 0 && ni < 6 && nj >= 0 && nj < 6 {
				if !v1[ni][nj] && f[ni][nj] == 1 {
					q1 = append(q1, Cell{ni, nj})
					v1[ni][nj] = true
				}
			}
		}
	...
	for len(q2) > 0 {
		...
		for k := 0; k < 4; k++ {
			ni, nj := c.i+di[k], c.j+dj[k]
			if ni >= 0 && ni < 6 && nj >= 0 && nj < 6 {
				if !v2[ni][nj] && f[ni][nj] == 0 {
					q2 = append(q2, Cell{ni, nj})
					v2[ni][nj] = true
				}
			}
		}
	}
	// 堀で囲まれた領域が1つの連結成分 かつ 穴がない
	return s1 == s2 && 36-s2 == s3
}
func main() {
