...
func SolveHonestly(r, g, b int) int {
	Divide := func(n int) (int, int) {
		if n%2 == 0 {
			return n / 2, n/2 - 1
		} else {
			return n / 2, n / 2
	...
	gl, gr := Divide(g)
	bl, br := Divide(b)
	return Sum(rl) + Sum(rr) + Sum(gl) + Sum(gr) + Sum(bl) + Sum(br)
}

func Solve(r, g, b int) int {
	const INF = 1 << 60
	//xにあるマーブルをl-rに並べる最小の手数
	F := func(x, l, r int) int {
		if l <= x && x <= r {
			return Sum(Abs(l)) + Sum(r)
		} else {
			n := r - l + 1
			var a int
			if l > x {
