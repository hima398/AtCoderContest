...
func FirstSolve(n int, s string) int {
	cm := make([][]int, 26)
	...
	for i := n - 1; i >= 0; i-- {
		...
		dp[i] %= p
		c := s[i] - 'A'
		//fmt.Println(string(s[i]), s[i], 'A', c)
		if len(cm[c]) > 0 {
			//fmt.Println(i, string(s[i]))
			ncm := len(cm[c])
			//最後に出てきた同じもじと現在の位置との差
			d := cm[c][ncm-1] - i - 1
			//fmt.Println(d)
			if d > 0 {
				x := Pow(2, d, p) - 1
...
func Solve(n int, s string) int {
	//i番目の文字まで見て、使用した文字の集合jで末尾の文字の種類がkである個数
	dp := make([][][10]int, n+1)
	mask := (1 << 10) - 1
	for i := 0; i <= n; i++ {
		dp[i] = make([][10]int, mask+1)
	}
	for i := 1; i <= n; i++ {
		x := s[i-1] - 'A'
		for j := 0; j <= mask; j++ {
			for k := 0; k < 10; k++ {
				dp[i][j][k] = dp[i-1][j][k]
				if x == byte(k) {
					// s[i]とi-1の末尾が同じであれば、i番目の文字を選ぶか選ばないかの
					// 種類がある。ので結果的に
					// dp[i][j][k] = 2*dp[i][j][k] となる
					dp[i][j][k] += dp[i-1][j][k]
					dp[i][j][k] %= p
				}
			}
