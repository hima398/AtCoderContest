...
func main() {
	...
	var Dfs func(i, pre, to int) bool
	Dfs = func(i, pre, to int) bool {
		//if pre >= 0 {
		//	d[pre][i]++
		//	d[i][pre]++
		//}
	...
	if (s+k)%2 == 1 || (s+k)/2 < 0 || (s-k)/2 < 0 {
		fmt.Println(0)
		return
	}
	r := Max((s+k)/2, (s-k)/2)
	dp := make([][]int, len(ds)+1)
	for i := 0; i <= len(ds); i++ {
		dp[i] = make([]int, r+1)
	}
	const p = 998244353
