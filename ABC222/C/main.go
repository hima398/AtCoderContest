...
func main() {
	...
	Eval := func(i1, i2 int, a1, a2 byte) (int, int) {
		//		fmt.Println(i1, i2, string(a1), string(a2))
		if a1 == a2 {
			return -1, -1
		} else if a1 == 'G' {
			if a2 == 'C' {
	...
	for j := 0; j < m; j++ {
		for i := 0; i < n; i++ {
			i1, i2 := ans[2*i].i, ans[2*i+1].i
			winner, _ := Eval(i1, i2, a[i1][j], a[i2][j])
