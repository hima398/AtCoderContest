...
func FirstSolve(n, m, k int, e map[int]map[int]bool) int {
	// k日立ってjにいる旅路の組み合わせ
	dp := make([][]int, k+1)
	for i := 0; i <= k; i++ {
		dp[i] = make([]int, n)
...
func main() {
	...
	for i := 0; i < m; i++ {
		...
		if me[u] == nil {
			me[u] = make(map[int]bool)
		}
		me[u][v] = true
		if me[v] == nil {
			me[v] = make(map[int]bool)
		}
		me[v][u] = true
	}

	//fmt.Println(FirstSolve(n, m, k, e))
	// 全通り求める
	dpa := make([][]int, k+1)
	// 壊れた道を使う経路数
	dpb := make([][]int, k+1)
	...
	for i := 1; i <= k; i++ {
		...
		for j := 0; j < n; j++ {
			dpa[i][j] = s - dpa[i-1][j]
			// 3重ループに見えるけどもiの中のループで合計で10000回程度しか回らないはず
			//for kk := 0; kk < len(e[j]); kk++ {
			//	dpa[i][j] -= dpa[i-1][e[j][kk]]
			//}
			for _, v := range e[j] {
				dpa[i][j] -= dpa[i-1][v]
			}
