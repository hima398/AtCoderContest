...
func main() {
	...
	for i := 0; i < 10000; i++ {
		...
		ok := true // 全てのキーが含まれる、xのキーが含まれない
		for _, v := range key {
			find := false
			for j := range number {
				if byte(v)+'0' == number[j] {
					find = true
				}
			}
			if !find {
	...
	/*
		s := nextString()
		a := Permutation(m["o"], m["o"]-1)
		ans := a * Combination(4, 4-m["o"]) * m["o"] / 2
		ans = a * Combination(4, 4-m["o"]) * m["?"]
