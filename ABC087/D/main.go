...
func Solve(n, m int, l, r, d []int) string {
	type Edge struct {
		t, d int
	}
	er, el := make([][]Edge, n), make([][]Edge, n)
	for i := 0; i < m; i++ {
		li, ri := l[i]-1, r[i]-1
		er[li] = append(er[li], Edge{ri, d[i]})
		el[ri] = append(er[ri], Edge{li, -d[i]})
	}

	ds := make([]int, n)
	visited := make([]bool, n)
	//書き方は3重ループになっているがO(N+M)くらいの見込み
	for i := 0; i < n; i++ {
		if !visited[i] {
			var q []Edge
			q = append(q, Edge{i, 0})
			...
			for len(q) > 0 {
				...
				q = q[1:]
				ds[p.t] = p.d
				for _, e := range er[p.t] {
					if !visited[e.t] {
						q = append(q, Edge{e.t, ds[p.t] + e.d})
						visited[e.t] = true
					}
				}
				for _, e := range el[p.t] {
					if !visited[e.t] {
						q = append(q, Edge{e.t, ds[p.t] + e.d})
						visited[e.t] = true
					}
...
func main() {
	...
	n, m := nextInt(), nextInt()
	l, r, d := make([]int, m), make([]int, m), make([]int, m)
	for i := 0; i < m; i++ {
		l[i], r[i], d[i] = nextInt(), nextInt(), nextInt()
	}
	ans := Solve(n, m, l, r, d)
	fmt.Println(ans)
}
