...
func main() {
	...
	// dp[0]同じ場所、dp[1]同じ行 dp[2]同じ列、dp[3]それ以外
	// 同じ
	dp[1][1] = 1
	dp[1][2] = 1
	for i := 2; i <= k; i++ {
		dp[i][0] = (w - 1) * dp[i-1][1] % p
		dp[i][0] += (h - 1) * dp[i-1][2] % p
		dp[i][0] %= p

		dp[i][1] = (w - 2) * dp[i-1][1] % p
		dp[i][1] += (h - 1) * dp[i-1][3] % p
		dp[i][1] += dp[i-1][0]
		dp[i][1] %= p

		dp[i][2] = (w - 1) * dp[i-1][3] % p
		dp[i][2] += (h - 2) * dp[i-1][2] % p
		dp[i][2] += dp[i-1][0]
		dp[i][2] %= p

		dp[i][3] = (h - 2) * dp[i-1][3] % p
		dp[i][3] += (w - 2) * dp[i-1][3] % p
