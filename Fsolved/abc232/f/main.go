...
func SolveCommentary(n, x, y int, a, b []int) int {
	const INF = 1 << 60
	pat := 1<<n - 1
	dp := make([]int, pat+1)
	for i := 0; i <= pat; i++ {
	...
	dp[0] = 0
	Inversion := func(s, x int) (ret int) {
		for i := 0; i < n; i++ {
			if (s>>i)&1 == 0 && i < x {
				ret++
			}
	...
	for s := 0; s <= pat; s++ {
		...
		for i := 0; i < n; i++ {
			...
			idx := s | (1 << i)
			dp[idx] = Min(dp[idx], dp[s]+Abs(a[i]-b[k])*x+Inversion(s, i)*y)
		}
	}
	return dp[pat]
}
