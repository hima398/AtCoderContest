...
func main() {
	...
	for i := 0; i < n; i++ {
		...
		for _, v := range s {
			idx := int(v - 'a')
			t = append(t, rune(x[idx]))
		}
		sn[i].t = t
	}
	sort.Slice(sn, func(i, j int) bool {
		if len(sn[i].t) == len(sn[j].t) {
			return string(sn[i].t) < string(sn[j].t)
		}
		return len(sn[i].t) < len(sn[j].t)
	})
	out := bufio.NewWriter(os.Stdout)
	defer out.Flush()
	for _, name := range sn {
		//fmt.Fprintln(out, string(name.s), string(name.t))
		fmt.Fprintln(out, string(name.s))
	}
}
