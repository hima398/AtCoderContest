...
func Solve(n int, x, y []int) int {
	...
	var ss []Section
	for i := 0; i < n; i++ {
		ss = append(ss, Section{x[i], y[i] - 1, x[i] - 1, y[i]})
	}
	sort.Slice(ss, func(i, j int) bool {
		return ss[i].ty*ss[j].tx < ss[j].ty*ss[i].tx
	})
	ans := 0
	cx, cy := ss[0].tx, ss[0].ty
	for i := 1; i < n; i++ {
		if cx*ss[i].sy < cy*ss[i].sx {
			ans++
			cx, cy = ss[i].tx, ss[i].ty
		}
	}
	return ans
