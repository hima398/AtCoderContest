...
func StrMin(s1, s2 string) string {
	if s1 < s2 {
		return s1
	}
	return s2
...
func SecoundSolve(n, k int, s []string) string {
	INF := strings.Repeat("z", 51)
	// とりあえずソートはしておく
	sort.Strings(s)
	//i番目(1-n)の文字列までのうちj個選んだ中で辞書順最小の文字列
	dp := make([][]string, n+1)
	for i := 0; i <= n; i++ {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < k; j++ {
			dp[i+1][j] = dp[i][j]
			fmt.Println(i, j, dp[i][j], s[i])
			dp[i+1][j+1] = StrMin(dp[i+1][j+1], dp[i][j]+s[i])
		}
...
func main() {
	...
	//fmt.Println(FirstSolve(n, k, s))
	fmt.Println(SecoundSolve(n, k, s))
}
