...
func FirstSolve() {
	...
	for len(q) > 0 {
		ball := q[0]
		q = q[1:]
		isRemoved[ball.c] = true
		p1, p2 := ball.ki1, ball.ki2

		if len(a[p1]) > 1 {
			a[p1] = a[p1][1:]
			b[a[p1][0]].d++
			if !isRemoved[a[p1][0]] && b[a[p1][0]].d >= 2 {
				q = append(q, b[a[p1][0]])
			}
		}
		...
		if len(a[p2]) > 1 {
			a[p2] = a[p2][1:]
			b[a[p2][0]].d++
			if !isRemoved[a[p2][0]] && b[a[p2][0]].d >= 2 {
				q = append(q, b[a[p2][0]])
			}
		}
