...
func main() {
	...
	bk := big.NewInt(0)
	bk.SetString(k, 10)
	bk.Sub(bk, big.NewInt(int64(pre)))
	mk := bk.Mod(bk, big.NewInt(int64(roop))).Int64()
	bk = big.NewInt(0)
	bk.SetString(k, 10)

	ss := 0
	if bk.Cmp(big.NewInt(int64(n))) <= 0 {
		ik := int(int(bk.Int64()))
		ss = Min(ik, pre+int(mk))
	} else {
		ss = pre + int(mk)
	}

	fmt.Println(pre, roop, ss, mk)
