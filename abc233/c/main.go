...
func FirstSolve(n, lx int, l []int, a [][]int) int {
	ans := 0
	...
	Dfs = func(x, i int) {
		...
		for j := 0; j < l[i]; j++ {
			// オーバーフローするものはxより大きい積になるのでここで止める
			//if x*a[i][j]/a[i][j] != x {
			if x > math.MaxInt64/a[i][j] {
				return
			}
			Dfs(x*a[i][j], i+1)
		}
...
func main() {
	...
	for i := 0; i < n; i++ {
		l[i] = nextInt()
		a[i] = nextIntSlice(l[i])
	}
	ans := 0
	var Dfs func(s, idx int)
	Dfs = func(s, idx int) {
		if idx == n {
			if s == lx {
				ans++
